...
from src.realtime.prompts import get_script_guidance_prompt, load_script
from src.validation.db import ValidationDB
...
class ScriptTester:
    ...
    def run_analysis(self, call_id: str) -> str:
        ...
        run_id = self.db.log_test_run(
            call_id=call_id, expected_id=exp_id, model=self.model, script_ver="v1.0", raw_output=response_str
        )
    ...
    def get_results(self, date_str: Optional[str] = None) -> List[AnalysisResult]:
        ...
        for row in rows:
            ...
            results.append(
                AnalysisResult(
                    call_id=row["call_id"], call_name=row["name"], passed=passed, notes=notes, raw_output=parsed
                )
            )
