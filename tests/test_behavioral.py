...
class TestBufferManagerBehavior(unittest.TestCase):
    ...
    def test_rotation_moves_active_to_context(self):
        ...
        self.manager.on_transcript_chunk("seg one", [
            {"text": "seg one", "start": 0.0, "end": 1.0, "completed": True},
        ])
        self.manager.on_transcript_chunk("seg two", [
    ...
    def test_no_llm_flooding_without_state_callback(self):
        ...
        mgr.on_transcript_chunk("World", [
            {"text": "World", "start": 1.0, "end": 2.0, "completed": True},
        ])
        self.assertEqual(
            len(callbacks), count_after_first,
    ...
    def test_sentence_end_triggers_analysis(self):
        ...
        self.manager.on_transcript_chunk("The price is too high.", [
    ...
    def test_context_buffer_trimming(self):
        ...
        for i in range(10):
            self.manager.on_transcript_chunk(f"seg {i}", [
                {"text": f"seg {i}", "start": float(i), "end": float(i + 1), "completed": True},
            ])
        self.assertLessEqual(
            len(self.manager.context_buffer), 3,
            "Context buffer must be trimmed to max_context_segments"
        )

    def test_reset_clears_everything(self):
        """reset() clears active, context, full_history, and tracking state."""
        self.manager.on_transcript_chunk("Hello", [
            {"text": "Hello", "start": 0.0, "end": 1.0, "completed": True},
        ])
        self.manager.on_transcript_chunk("World", [
            {"text": "World", "start": 1.0, "end": 2.0, "completed": True},
        ])
        self.manager.reset()
    ...
    def test_incomplete_segment_included_in_payload(self):
        ...
        self.manager.on_transcript_chunk("Hello partial", [
            {"text": "Hello", "start": 0.0, "end": 1.0, "completed": True},
    ...
    def test_processed_keys_pruned_on_rotation(self):
        ...
        for i in range(20):
            self.manager.on_transcript_chunk(f"s{i}", [
...
class TestConnectionManagerBehavior(unittest.TestCase):
    ...
    def test_connect_sends_history(self):
        """New connections receive full transcript history."""
    ...
    def test_broadcast_stores_transcripts(self):
        """Broadcast stores transcript messages in history."""
        from src.web.app import ConnectionManager
        mgr = ConnectionManager()
        ws = self._make_mock_ws()
        ...
        ws.send_json.assert_called_with(msg)
    ...
    def test_reset_clears_history_and_notifies(self):
        """reset() clears transcript history and sends reset to all connections."""
        from src.web.app import ConnectionManager
        mgr = ConnectionManager()
        ws = self._make_mock_ws()
        mgr.active_connections = [ws]
    ...
    def test_has_connection_timeout(self):
        ...
        from src.web.app import ConnectionManager
        mgr = ConnectionManager()
        self.assertLessEqual(mgr.connection_timeout, 5.0,
                             "Connection timeout must be <= 5s for drop detection")
...
class TestStreamingAnalyzerBehavior(unittest.TestCase):
    ...
    def test_fallback_triggers_on_timeout(self):
        ...
        analyzer = StreamingAnalyzer(
            api_key="test", base_url="http://fake",
            model="slow-model", fallback_model="fast-model"
        )
    ...
    def test_fallback_raises_without_fallback_model(self):
        ...
        analyzer = StreamingAnalyzer(
            api_key="test", base_url="http://fake",
            model="slow-model", fallback_model=None
        )
...
class TestAnalysisOrchestratorBehavior(unittest.TestCase):
    ...
    def test_submit_triggers_callback(self):
        ...
        from src.realtime.analysis_orchestrator import StreamingAnalyzer, AnalysisOrchestrator

        analyzer = StreamingAnalyzer(api_key="test", base_url="http://fake", model="m")

        mock_response = iter([MagicMock(choices=[MagicMock(delta=MagicMock(content='{"script_location": "Opening", "key_points": [], "suggestion": "test"}'))])])
        analyzer.client = MagicMock()
        ...
        try:
            ...
            def make_response(*args, **kwargs):
                chunk = MagicMock()
                chunk.choices = [MagicMock(delta=MagicMock(content='{"script_location": "Opening", "key_points": [], "suggestion": "test"}'))]
                return iter([chunk])
...
class TestSummaryEngineBehavior(unittest.TestCase):
    ...
    def test_transcript_bounded(self):
        ...
        self.assertLessEqual(
            len(lines), engine._max_transcript_lines,
